...
# 1. Add PaymentUpdate Schema right after PaymentCreate
payment_create_re = r'class PaymentCreate\(BaseModel\):.*?    apply_gst: bool = False\n'
new_payment_update = """class PaymentUpdate(BaseModel):
...
# find PaymentCreate
m = re.search(payment_create_re, content, re.DOTALL)
if m:
    content = content[:m.end()] + "\n" + new_payment_update + content[m.end():]

# 2. Add PATCH /payments/{payment_id} right after DELETE /payments/{payment_id}
delete_re = r'    @erp\.delete\("/payments/\{payment_id\}"\).*?return \{"ok": True, "deleted_id": payment_id\}\n'
new_patch = """
    ...
    async def update_payment(payment_id: str, payload: PaymentUpdate, user: dict = Depends(require_super)):
        ...
        # Recalculate student totals if needed? No, student total is calculated dynamically in student_statement
...
m = re.search(delete_re, content, re.DOTALL)
if m:
    content = content[:m.end()] + new_patch + content[m.end():]
